...
const Update = () => {
  ...
  useEffect(() => {
    fetch(`/real_state/${params.id}`)
      .then((res) => res.json())
      .then(
        (data) => {
          console.log(data.data);
          setInputs(data.data);
          //   const arrayData = Object.keys(data.data[0]).map((key,index) => {
          //     setInputs((values) => ({ ...values, [key]: data.data[0][key] }));
  ...
  const handleSubmit = (event) => {
    ...
    fetch(`/real_state/${params.id}`, options)
      .then((res) => res.json())
      .then(
        (data) => {
